...
export function calculateRiskScore(
  ...
  return (Math.min(score, 100.0) as f64) as i32
}
...
export function createMEVAlert(
  ...
  alert.severity = riskScore > 75 ? "CRITICAL" : (riskScore > 50 ? "HIGH" : (riskScore > 25 ? "MEDIUM" : "LOW"))
  alert.targetAddress = transaction.to || Address.zero()
  ...
  alert.riskScore = BigInt.fromI32(riskScore)
  alert.detected = true
  ...
  let avgGas = BigInt.fromI32(20000000000) // 20 gwei default
...
export function analyzeMEVPatterns(
  ...
  transaction.isMEV = riskScore > 50
  transaction.riskScore = BigInt.fromI32(riskScore)
  transaction.save()
