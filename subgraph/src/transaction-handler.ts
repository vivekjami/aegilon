...
export function handleBlock(block: ethereum.Block): void {
  ...
  // Note: Individual transactions are processed through contract event handlers
  // This block handler is used for block-level analytics and cleanup
...
export function handleTransactionReceipt(
  ...
  if (status.equals(BigInt.fromI32(1))) { // Transaction successful
...
export function batchProcessTransactions(transactions: Transaction[]): void {
  ...
  log.info("Batch processing completed for {} transactions", [transactions.length.toString()])
}
